...
def run_forecast(forecast_config, today, throughput_history, monte_carlo_service, work_item_service):
...
def create_forecasts(config, history, today, work_items, work_item_filter_service, work_item_service):    
    ...
    for forecast_config in forecasts:                    
        run_forecast(forecast_config, today, throughput_history, monte_carlo_service, work_item_service)
...
def create_chart(chart_config, work_items, flow_metrics_service):
    ...
    match chart_type.lower():
        ...
        case "processbehaviourcharts":
            ...
            flow_metrics_service.plot_throughput_process_behaviour_chart(work_items, baseline_start, baseline_end, chart_config["throughputChartName"])            
            flow_metrics_service.plot_wip_process_behaviour_chart(work_items, baseline_start, baseline_end, chart_config["wipChartName"])
            flow_metrics_service.plot_cycle_time_process_behaviour_chart(work_items, baseline_start, baseline_end, chart_config["cycleTimeChartName"])       
            flow_metrics_service.plot_total_age_process_behaviour_chart(work_items, baseline_start, baseline_end, chart_config["itemAgeChartName"])                        
...
def create_charts(config, today, history, work_items, work_item_filter_service):
    ...
    flow_metrics_service = FlowMetricsService(show_plots, charts_folder, work_item_filter_service, history, plot_labels, today)
...
def run_tool_for_config_file(config_path):
    ...
    work_item_service = WorkItemServiceFactory().create_service(data_source, data_sources, history, today)
    ...
    create_forecasts(config, history, today, work_items, work_item_filter_service, work_item_service)
