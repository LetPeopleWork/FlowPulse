...
class WorkItemServiceFactory:
    def create_service(self, data_source_type, data_sources, history_in_days, today):
        ...
        data_source = next((ds for ds in data_sources if ds["name"].lower() == data_source_type.lower()), None)
        if not data_source:
        ...
        match data_source_type:
            ...
            case _:
                raise ValueError(f"Work Tracking System '{data_source_type}' not supported. Supported values are 'Jira', 'Azure DevOps' and 'CSV'")
