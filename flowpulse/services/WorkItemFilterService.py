...
class WorkItemFilterService:
    ...
    def get_in_progress_items(self, work_items):
        ...
        return [item for item in work_items 
            if item.started_date and item.started_date <= self.today and (not item.closed_date or (item.closed_date > start_date and item.closed_date <= self.today))]
        
    def get_items_opened_in_period(self, work_items):
        start_date = self.today - timedelta(days=self.history_days)
        
        return [item for item in work_items 
                if item.started_date and item.started_date <= self.today and item.started_date >= start_date]